    ...
    \begin{figure}[t]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/architecture-overview.png}
    \caption{End-to-end system architecture of the AI Sales Agent Platform.}
    \end{figure}
    ...
    \begin{figure}[t]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/ingestion-flow.png}
    \caption{Document ingestion workflow: parse, chunk, embed, and store.}
    \end{figure}
    ...
    \subsubsection{LangGraph-Orchestrated Workflow}
    We implement the agent using LangGraph as a stateful workflow with an explicit tool-use loop. The agent state includes the conversation messages, system prompt, agent identifier, and retrieved context. Each turn executes an LLM node that can either respond directly or emit tool calls. If a tool call is emitted, control flows to a tool node that executes the requested action (e.g., \texttt{search\_knowledge\_base}, \texttt{send\_email}, booking tools), then appends the tool result to the message history. The graph then returns to the LLM node, enabling multi-step reasoning over tool outputs before producing a final answer. This design enforces a structured decision process: generate $\rightarrow$ call tool $\rightarrow$ observe $\rightarrow$ answer. It also supports per-agent customization by injecting agent-specific system prompts and tool schemas into the workflow at initialization.

    \begin{figure}[t]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/chat-flow.png}
    \caption{Query and generation workflow with hybrid retrieval and tool use.}
    \end{figure}
